...
void devIterate_direction( const int dirx, const int diry, const int *left_image,
                        const int* costs, int *accumulated_costs, 
                        const int nx, const int ny, const int disp_range ); 
                        ...
                        const int nx, const int ny, const int disp_range ) 
{
   ...
   // const int WIDTH = nx;
    const int HEIGHT = ny;
    int d = threadIdx.x + blockIdx.x * blockDim.x;
    int i = threadIdx.y + blockIdx.y * blockDim.y; // verificar se j < ny
...
/*//////////////////////////////////////////////////////////////////////////////////////////////////////*/
/*				FUNCAO KERNEL TASK 2 	- OU ENTÃO NÃO...										        */
/*//////////////////////////////////////////////////////////////////////////////////////////////////////*/
...
/*
	int block2x_x = 1;
  	int block2x_y = nx; 
  	//int block2x_z = disp_range; // 512
  	int grid2x_x = 1;
  	int grid2x_y = ceil((float)ny/block2x_y);
	dim3 dimBlock2x(block2x_x,block2x_y);
	dim3 dimGrid2x(grid2x_x,grid2x_y);
  
  	int block2y_x = ny;
  	int block2y_y = 1; 
  	//int block2y_z = disp_range;  // 512
 	int grid2y_x = ceil((float)ny/block2y_x);
  	int grid2y_y = 1;
	dim3 dimBlock2y(block2y_x,block2y_y);
  	dim3 dimGrid2y(grid2y_x,grid2y_y);
*/

  	////imagens ficam identifcas
  	int block_x1 = disp_range;
    int block_y1 = 1;

    int block_x2 = 1;
    int block_y2 = disp_range;
  //  int grid_x = ceil( (float)nx / block_x);
  //  int grid_y = ceil( (float)ny / block_y);
 
   // dim3 grid(grid_x, grid_y);
    dim3 block1(block_x1, block_y1); 
    dim3 block2(block_x2, block_y2); 
    ...
    if ( dirx > 0 ) {
      	devIterate_direction_dirxpos<<<grid_nx, block1>>> (dirx,devLeft_image,devCosts,devAccumulated_costs, nx, ny, disp_range);
      	cudaMemcpy(accumulated_costs, devAccumulated_costs, nx*ny*sizeof(int)*disp_range, cudaMemcpyDeviceToHost);

    }
    else if ( diry > 0 ) {
       iterate_direction_dirypos(diry,left_image,costs,accumulated_costs, nx, ny, disp_range);
    	//devIterate_direction_dirypos<<<grid_ny, block1>>> (diry,devLeft_image,devCosts,devAccumulated_costs, nx, ny, disp_range);
    	//cudaMemcpy(accumulated_costs, devAccumulated_costs, nx*ny*sizeof(int)*disp_range, cudaMemcpyDeviceToHost);

    } 
    else if ( dirx < 0 ) {
        //iterate_direction_dirxneg(dirx,left_image,costs,accumulated_costs, nx, ny, disp_range);
      	devIterate_direction_dirxneg<<<grid_nx, block1>>> (dirx,devLeft_image,devCosts,devAccumulated_costs, nx, ny, disp_range);
	      cudaMemcpy(accumulated_costs, devAccumulated_costs, nx*ny*sizeof(int)*disp_range, cudaMemcpyDeviceToHost);
        ...
        iterate_direction_diryneg(diry,left_image,costs,accumulated_costs, nx, ny, disp_range);
      	...
      	//devIterate_direction_diryneg<<<grid_ny, block>>> (diry,devLeft_image,devCosts,devAccumulated_costs, nx, ny, disp_range);
    }
...
void sgmDevice( const int *h_leftIm, const int *h_rightIm,  int *h_dispIm, const int w, const int h, const int disp_range ){
  ...
  int block_z = disp_range; // 512
	...
	devDetermine_costs<<<dimGrid, dimBlock>>>(devH_leftIm, devH_rightIm, devCosts, nx, ny, disp_range);
	
	/*copiar output para o codigo C*/
	cudaMemcpy(costs, devCosts, memsize*disp_range, cudaMemcpyDeviceToHost);
	...
	for(dirx=-1; dirx<2; dirx++) {
		if(dirx==0 && diry==0) continue;
		std::fill(dir_accumulated_costs, dir_accumulated_costs+nx*ny*disp_range, 0);
		devIterate_direction( dirx,diry, h_leftIm, costs, dir_accumulated_costs, nx, ny, disp_range);
		inplace_sum_views( accumulated_costs, dir_accumulated_costs, nx, ny, disp_range);
	}
	dirx=0;
	for(diry=-1; diry<2; diry++) {
